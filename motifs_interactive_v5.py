...
DEFAULT_PARAMS = {
    ...
    "detail_boost":   0.0,     # high-pass sharpening (0 = off, amplifies striations too)
    ...
    "mesh_blur_sigma": 1.5,    # Gaussian blur on depth before mesh (0=off)
    "depth_detrend":   True,   # subtract best-fit surface from depth (fixes flash/lighting tilt)
    "depth_bilateral": 20.0,   # bilateral filter sigma — kills striations, keeps edges (0=off)
    "aspect_crop":     True,   # crop to mask bounding box
    "crop_pad_px":     20,     # padding around crop in px
}
...
def run_depth(model, image_path):
    ...
    if (src_h > src_w) != (d_h > d_w):
        ...
        print(f"  [INFO] Depth rotated 90 CW to match source orientation")
    return depth
...
def preprocess(image_path, depth, p):
    ...
    # Keep largest connected component — but skip if mask is near-full
    # (pre-cropped images have no background to exclude)
    coverage_pre = mask.sum() / (255.0 * mask.size)
    if coverage_pre < 0.92:
        ...
        print(f"  [INFO] Mask coverage {coverage_pre:.0%} — skipping component selection (pre-cropped image)")
...
def build_heightfield(d2_smooth, mask_erode, p):
    ...
    # ── Depth detrend: fit and subtract a surface ──────────────────────
    # Fixes flash/uneven lighting that DA3 misreads as depth tilt.
    # Uses quadratic surface (handles curved tubelight gradients) and
    # excludes a 10% border margin so edge artifacts don't skew the fit.
    if p.get("depth_detrend", False):
        h_d, w_d = dn.shape
        xs = np.linspace(0, 1, w_d); ys = np.linspace(0, 1, h_d)
        XX, YY = np.meshgrid(xs, ys)

        # Build fit region: mask AND exclude 10% border
        margin_y = max(1, int(0.10 * h_d))
        margin_x = max(1, int(0.10 * w_d))
        interior = np.zeros_like(m, dtype=bool)
        interior[margin_y:-margin_y, margin_x:-margin_x] = True
        fit_region = m & interior
        if fit_region.sum() < 500:
            fit_region = m   # fallback if too few pixels

        flat = dn[fit_region].flatten()
        xf = XX[fit_region].flatten()
        yf = YY[fit_region].flatten()
        # Quadratic: ax² + by² + cxy + dx + ey + f
        A = np.stack([xf**2, yf**2, xf*yf, xf, yf,
                      np.ones(len(xf))], axis=1)
        try:
    ...
    # ── Edge taper: smooth transition at mask boundary ────────────────
    # Prevents the hard cliff at mask edges that causes edge clamping.
    # Creates a 15-pixel gradient from full height to zero at the mask border.
    if m.sum() > 0 and m.sum() < m.size:
        dist    = cv2.distanceTransform(m.astype(np.uint8), cv2.DIST_L2, 5)
        taper_w = 15.0
        taper   = np.clip(dist / taper_w, 0.0, 1.0)
        dn      = dn * taper

    # ── Bilateral filter — kills striations, keeps carving edges ──────
    bil_sigma = p.get("depth_bilateral", 0.0)
    if bil_sigma > 0:
        d_u8 = (dn * 255).astype(np.uint8)
        # Two passes: first pass kills broad banding, second refines
        d_u8 = cv2.bilateralFilter(d_u8, d=11,
                                   sigmaColor=float(bil_sigma),
                                   sigmaSpace=float(bil_sigma))
        d_u8 = cv2.bilateralFilter(d_u8, d=7,
                                   sigmaColor=float(bil_sigma * 0.5),
                                   sigmaSpace=float(bil_sigma * 0.5))
        dn = d_u8.astype(np.float32) / 255.0

    # ── Pre-mesh Gaussian blur ────────────────────────────────────────
...
PARAM_HELP = {
    ...
    "detail_boost":   "High-pass sharpening (0=off RECOMMENDED for shallow relief — amplifies striations)",
    ...
    "z_exag":          "Z exaggeration multiplier (1-30 normal, 60-100 for shallow relief)",
    "mesh_blur_sigma": "Gaussian blur on depth before mesh (0=off, 1-3 smooths striations)",
    "depth_detrend":   "Subtract best-fit quadratic surface — ESSENTIAL for lit panels (True/False)",
    "depth_bilateral": "Bilateral filter sigma (default 20, 0=off, 15-35 kills banding keeps edges)",
...
def process_image(model, image_path, out_dir, params):
    ...
    prep_path = f"{base_path}_preprocessed{ext}"
    if os.path.exists(prep_path):
        print(f"  [Preprocessor] Found: {os.path.basename(prep_path)}")
        if input("  Use preprocessed image? [y/n]: ").strip().lower() == "y":
            image_path = prep_path
    else:
        print(f"  Tip: py preprocess_live.py --image {os.path.basename(image_path)}")
    name = os.path.splitext(os.path.basename(image_path))[0].replace("_preprocessed", "")
...
def main():
    ...
    parser.add_argument("--output", default="./motifs/finals", help="Folder for saved STL files")
